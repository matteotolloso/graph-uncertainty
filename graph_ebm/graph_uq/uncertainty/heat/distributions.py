...
class Categorical(torch.distributions.Categorical):
    def sample(self, sample_shape=torch.Size(), target=None):
        ...
        probs_2d = self.probs.reshape(-1, self._num_events)  # type: ignore
...
class MixtureSameFamily(torch.distributions.MixtureSameFamily):
    ...
    def log_prob(self, x, labels=None, reduce="logsumexp"):
        ...
        assert reduce in [
            "logsumexp",
            "min",
        ], f"Got reduce={reduce}, expected one of ['logsumexp', 'min']"
        ...
        if reduce == "logsumexp":
            if labels is None:
                ...
                log_mix_prob = torch.log_softmax(
                    self.mixture_distribution.logits[labels], dim=-1
                )  # [B, k]
