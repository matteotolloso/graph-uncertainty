...
import graph_uq.model.gpn.distributions as UD
...
def balanced_loss_weights(
    ...
    counts = torch_scatter.scatter_sum(
        torch.ones_like(labels), labels, dim_size=num_classes
    )
    num_classes = counts.size(0)
    effective_counts = 1 - beta / (
        1 - torch.exp(counts * torch.log(torch.tensor(beta, device=counts.device)))
    )
    if normalize:
        effective_counts *= (
            num_classes / effective_counts[torch.isfinite(effective_counts)].sum()
        )
...
@jaxtyped(typechecker=typechecked)
def uce_loss(
    alpha: Float[Tensor, "num_nodes num_classes"], y: Int[Tensor, "num_nodes"]
...
def entropy_regularization(
    alpha: Float[Tensor, "num_nodes num_classes"], approximate: bool = True
) -> Float[Tensor, "num_nodes"]:
